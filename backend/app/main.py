...
async def create_dataset(request: Request):
    ...
    try:
        ...
        if not isinstance(simulation_params, dict) or len(simulation_params) == 0:
            return api_error(
                message="Error: Missing or invalid simulation params",
                status_code=400,
                code="INVALID_PARAMS",
            )
        ...
        return api_error(
            message="Unexpected server error while generating data",
            status_code=500,
...
async def list_datasets(request: Request):
    try:
        ...
        return api_success(
            message="Success: Datasets retrieved successfully",
            data={"datasets": dataset_names, "count": len(dataset_names)},
            status_code=200,
        )

    except UnicodeDecodeError:
        return api_error(
            message=f"Error: Could not find dataset list",
            status_code=500,
        ...
        return api_error(
            message="Unexpected server error while reading datasets",
            status_code=500,
...
async def dataset_dashboard(dataset_name: str, request: Request):
    try:
        ...
        missing = []
        if not trees_path.exists(): missing.append(trees_path.name)
        if not truth_path.exists(): missing.append(truth_path.name)
        if not observed_path.exists(): missing.append(observed_path.name)

        if missing:
            return api_error(
                message=f"Missing required files for dataset '{dataset_name}': {', '.join(missing)}",
                status_code=404,
        ...
        return api_error(
            message="Error: Could not decode one of the CSV files (encoding issue)",
            status_code=500,
        ...
        return api_error(
            message="Unexpected server error while building dashboard response",
            status_code=500,
