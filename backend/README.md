...
| **`datasets/*.pedigree.txt`**           | Mapping file connecting individuals to their parents and their time (generation) in the simulation.                     | `individual_id`, `time`, `parent_0_id`, `parent_1_id`, `num_nodes`. |
...
| **`datasets/*.relationship.svg`**       | Visual representation of the first tree in the sequence, highlighting known vs. masked individuals.                     | Blue nodes (Known) vs. Dotted Outlines (Masked).                    |
